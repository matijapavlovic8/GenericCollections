...
public class ArrayIndexedCollection<T> implements List<T> {
	...
	public void remove(int index) {
        ...
        if (this.size - 1 - index >= 0) System.arraycopy(this.elements, index + 1, this.elements, index, this.size - 1 - index);
    ...
    private static class ArrayCollectionElementsGetter<T> implements ElementsGetter<T> {
        private final ArrayIndexedCollection<T> coll;
        private int nextIndex;
        private long savedModificationCount;

        private ArrayCollectionElementsGetter(ArrayIndexedCollection<T> coll){
